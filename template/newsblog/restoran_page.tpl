...
<section class="video">
	<?php if ($addimages){ ?>
		<div class="swiper" id="rest_swiper">
	...
	<script>
		$(document).ready(function(){
			var swiper = new Swiper("#rest_swiper", {
				loop: true,
				navigation: {
					nextEl: ".btn--next",
					prevEl: ".btn--prev",
	...
	<div class="wrapper">
		...
		<div class="head d-flex justify-between">
			<div class="head-title uppercase">
				<div class="subtitle mar-none">Ресторан</div>
		...
		<?php if ($addimages){ ?>
			<div class="news-list d-grid lg-col-2 md-col-2 sm-col-1 xs-col-1 row-gap-20 column-gap-20">
				<?php foreach ($addimages as $addimage) { ?>
					<?php if ($addimage['addhead'] == 'Слайдер') { ?>
					<?php } else { ?>
						<div class="banket-item radius-large overflow relative d-block image_hover">
							<?php if($addimage['additional']){ ?>
								<?php if($addimage['addvideo']){ ?>
									<iframe width="800" height="540" src="https://www.youtube.com/embed/<?php echo $addimage['addvideo']; ?>" class="img-responsive h100" frameborder="0" allowfullscreen></iframe>
									<?php } else { ?>
							...
							<div class="caption d-flex direction-column w100">
								<?php if($addimage['addhead'] == 'Основное меню'){ ?>
									<div class="text-pdf d-flex justify-between align-center column-gap-20">
										<div class="name subtitle uppercase"><?php echo $addimage['addhead']; ?></div>
										<?php if($addimage['addtext']){ ?>
											<a href="/<?php echo $addimage['addtext']; ?>" class="icon-pdf" target="_blank"></a>
										<?php } ?>
									</div>
								<?php } else if($addimage['addhead'] == 'Барное меню') { ?>
									<div class="text-pdf d-flex justify-between align-center column-gap-20">
										<div class="name subtitle uppercase"><?php echo $addimage['addhead']; ?></div>
										<?php if($addimage['addtext']){ ?>
											<a href="/<?php echo $addimage['addtext']; ?>" class="icon-pdf" target="_blank"></a>
